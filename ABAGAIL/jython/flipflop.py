...
for i in range(seeds):
    for N in runs:
        ...
        fit = ConvergenceTrainer(rhc, N, sys.maxint) if (converge) else FixedIterationTrainer(rhc, 1500000)
        t0 = time.time()
        fit.train()
        t1 = time.time()
        ...
        fit = ConvergenceTrainer(sa, N-1, sys.maxint) if (converge) else FixedIterationTrainer(sa, 1500000)
        t0 = time.time()
        fit.train()
        t1 = time.time()
        calls = ef.functionCalls
        ...
        fit = ConvergenceTrainer(ga, N-1, sys.maxint) if (converge) else FixedIterationTrainer(ga, 650)
        t0 = time.time()
        fit.train()
        t1 = time.time()
        calls = ef.functionCalls
        ...
        population = 2500
        # Train MIMIC
        mimic = MIMIC(population, int(population*0.5), pop)
        fit = ConvergenceTrainer(mimic, N-1, sys.maxint) if (converge) else FixedIterationTrainer(mimic, 600)
        t0 = time.time()
        fit.train()
        t1 = time.time()
        calls = ef.functionCalls
...
with open('../../flipflop.json', 'w') as fp:
    json.dump(flipflop, fp)
